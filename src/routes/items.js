...
router.get('/:id', async (req, res) => {
  try {
    ...
    const item = items[0];
    const user = req.session.user;            // detect if the user is admin or not
    const isAdmin = user && user.u_role === 'admin';
    if (isAdmin) {
  console.log('Opened itemDetailAdmin for item', itemId);
}
    const viewName = isAdmin ? 'itemDetailAdmin' : 'itemDetail';
